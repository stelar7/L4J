...
public abstract class RateLimiter
{
    ...
    {
        /*
         * We always hold the mutex when calling this. TODO(cpovirk): Is that important? Perhaps we need to guarantee that each call to reserveEarliestAvailable, etc. sees a value >= the previous? Also, is it OK that we don't hold the mutex when sleeping?
         */
    ...
    /**
     * Creates a {@code RateLimiter} with the specified stable throughput, given as "permits per second" (commonly referred to as <i>QPS</i>, queries per second).
     *
     * <p>
     ...
     *            the rate of the returned {@code RateLimiter}, measured in how many permits become available per second
     ...
     *            the rate of the returned {@code RateLimiter}, measured in how many permits become available per second
     * @param warmupPeriod
     *            the duration of the period where the {@code RateLimiter} ramps up its rate, before reaching its stable (maximum) rate
     * @param unit
     *            the time unit of the warmupPeriod argument
     * @throws IllegalArgumentException
     *             if {@code permitsPerSecond} is negative or zero or {@code warmupPeriod} is negative
     */
    ...
    /**
     * Reserves the given number of permits from this {@code RateLimiter} for future use, returning the number of microseconds until the reservation can be consumed.
     *
     ...
     *
     * <p>
     * Note though that, since each request repays (by waiting, if necessary) the cost of the <i>previous</i> request, this means that the very next request after an invocation to {@code setRate} will not be affected by the new rate; it will pay the cost of the previous request, which is in terms of the previous rate.
     *
     * <p>
     * The behavior of the {@code RateLimiter} is not modified in any other way, e.g. if the {@code RateLimiter} was configured with a warmup period of 20 seconds, it still has a warmup period of 20 seconds after this method invocation.
     *
     * @param permitsPerSecond
    ...
    /**
     * Acquires the given number of permits from this {@code RateLimiter} if it can be obtained without exceeding the specified {@code timeout}, or returns {@code false} immediately (without waiting) if the permits would not have been granted before the timeout expired.
     *
     * @param permits
     *            the number of permits to acquire
     * @param timeout
     *            the maximum time to wait for the permits. Negative values are treated as zero.
     * @param unit
     ...
     *
     * @param timeout
     *            the maximum time to wait for the permit. Negative values are treated as zero.
     * @param unit
     *            the time unit of the timeout argument
     * @return {@code true} if the permit was acquired, {@code false} otherwise
     * @throws IllegalArgumentException
